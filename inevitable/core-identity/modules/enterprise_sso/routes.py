...
class SSOProviderCreate(SecureBaseModel):
...
class SSOProviderUpdate(SecureBaseModel):
...
class SSOTestRequest(SecureBaseModel):
...
async def oauth_login(
    ...
    # CRITICAL FIX: Validate redirect URI against whitelist
    # Prevents open redirect attacks
    from urllib.parse import urlparse
    
    # Get allowed redirect URIs from provider config
    allowed_redirects = provider.config.get("redirect_uris", [])
    
    # Parse the provided redirect URI
    parsed_uri = urlparse(redirect_uri)
    
    # Strict validation - must match exactly or be a subdirectory of allowed URIs
    is_valid = False
    for allowed in allowed_redirects:
        ...
        # Check if it's a valid subdirectory (same scheme, netloc, and starts with path)
...
async def oauth_callback(
    ...
    """OAuth/OIDC callback"""
    if error:
        # CRITICAL FIX: Sanitize error parameters to prevent response splitting
        # Remove any control characters that could cause header injection
        import re
        
        def sanitize_error_param(param: Optional[str]) -> str:
            if not param:
                return "unknown"
            # Remove any newlines, carriage returns, and other control characters
            sanitized = re.sub(r'[\r\n\x00-\x1f\x7f-\x9f]', '', param)
            # Limit length to prevent DoS
            return sanitized[:200] if sanitized else "unknown"
        
        safe_error = sanitize_error_param(error)
        safe_description = sanitize_error_param(error_description)
        
        logger.warning(f"OAuth error: {safe_error} - {safe_description}")
        raise HTTPException(400, f"OAuth error: {safe_error}")
    
    # Validate state
    # CRITICAL FIX: Sanitize state parameter before use
    import re
    if not re.match(r'^[a-zA-Z0-9_-]+$', state):
        logger.warning(f"Invalid state format attempted: {state[:50]}")
        raise HTTPException(400, "Invalid state parameter format")
