...
class PricingPlanUpdate(SecureBaseModel):
...
class MigrationCreate(SecureBaseModel):
    subscription_id: str
    target_plan_id: int
...
class BulkMigrationCreate(SecureBaseModel):
    name: str
...
class TransitionRuleCreate(SecureBaseModel):
    name: str
    source_plan_id: int
    target_plan_id: int
...
class SubscriptionUpdate(SecureBaseModel):
    plan_id: Optional[int] = None
    cancel_at_period_end: Optional[bool] = None
    metadata: Optional[Dict[str, Any]] = None


class GrandfatherRequest(SecureBaseModel):
    plan_id: int
    criteria: Optional[Dict[str, Any]] = None
...
async def get_subscription_details(
    ...
    # CRITICAL FIX: Add tenant isolation to prevent cross-tenant access
    # Get from local database with tenant filtering
    package = db.query(Package).join(Customer).filter(
        Package.stripe_subscription_id == subscription_id,
        Customer.tenant_id == admin_user.tenant_id
    ).first()
    
    if not package:
        raise HTTPException(status_code=404, detail="Subscription not found")
    
    # Get customer (already filtered by tenant via join)
